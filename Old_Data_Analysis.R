setwd('C:/Users/aywel/Desktop/Research/Barabasi Lab/Nomix/Code/Data (Old)')
...
data.combo <- read.table("Total_Group.txt")    # RAW DATA
data.pheno <- read_excel("Pheno.xlsx")    # Batch numbers for each sample number
data.batch <- as.data.frame(data.pheno[,2])    # Just batch numbers

# Trial Analysis ---------------------------------------------------------------

      # (Need better and more raw_count patients)
raw_counts <- matrix(as.matrix(data.combo[,2:602]), ncol = ncol(data.combo[,2:602]), dimnames = NULL)
rownams <- (data.combo[,1])
colnams <- colnames(data.combo)
